...
| `/auth/invitatie/[token]` | Link unic din WhatsApp/SMS. Emailul e precompletat din invitație; terapeutul alege **doar parola** și apasă activarea. Dacă adresa există deja în Supabase Auth, serverul **setează parola** și confirmă emailul (nu eșuează ca la un signUp). Altfel creează contul (`email_confirm: true`, **fără OTP**). Apoi face **UPDATE** pe `therapist_invites` (`accepted_user_id`, `accepted_at`), asociază terapeutul de clinică și deschide sesiunea pe **același** client — destinația e **doar** `/dashboard`, niciodată `/onboarding`. Erorile Supabase sunt logate cu `[invite-activate]` și afișate în formular. **Google** rămâne o alternativă: după OAuth, asocierea e pe token sau emailul pending, tot spre `/dashboard`. |
