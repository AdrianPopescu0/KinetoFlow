...
- `EARLY_ACCESS_CODE` — **exact 12 caractere**. Codul secret pentru butonul Early Access de pe landing. Dacă lipsește sau nu are 12 caractere, local se folosește `KINETO-EARLY`. Pe producție puneți propriul cod. După validare, Server Action-ul setează cookie-ul HTTP-only `early_access_verified` (90 de zile, `path: /`). Middleware-ul îl verifică **înainte de orice altceva**, deci taburile noi nu mai cer codul. Semnătura e identică pe Node și Edge; `EARLY_ACCESS_PEPPER` e opțional doar pentru cookie-uri vechi.
...
- Middleware care reîmprospătează sesiunea, blochează `/dashboard/*` pentru vizitatori, și cere cookie-ul HTTP-only `early_access_verified` (**90 de zile**, verificat **primul** în `middleware.ts`) pentru `/login` dacă nu există sesiune. Conturile cu clinică merg direct în `/dashboard`.
