...
- Middleware care reîmprospătează sesiunea, blochează `/dashboard/*` pentru vizitatori, și cere cookie-ul HTTP-only `early_access_verified` (**90 de zile**; dacă există pe request, utilizatorul trece la `/login` / `/dashboard`) dacă nu există sesiune. Conturile cu clinică merg direct în `/dashboard`.
