...
- Opțional, pentru **emailuri** (suport + OTP la înregistrare): `RESEND_API_KEY`. Codul de 6 cifre pleacă de pe `no-reply@kinetoflow.ro` (`RESEND_AUTH_FROM`) **doar la crearea contului**, ca text, fără buton de confirmare. La Sign In, utilizatorul confirmat introduce doar email și parolă. Fără cheie, în dezvoltare codul e logat.
...
OTP de confirmare email (doar la înregistrare, inclusiv terapeuți invitați): rulează `sql/026_auth_email_otps.sql` (sau `supabase/migrations/019_auth_email_otps.sql`). La crearea contului, serverul generează OTP-ul prin `auth.admin.generateLink` și îl trimite **doar ca text** (6 cifre vizibile) de pe `KinetoFlow <no-reply@kinetoflow.ro>` (`RESEND_AUTH_FROM`). Nu include un buton/link de autologin. Utilizatorul tastează codul pe `/auth/email-cod` (clinică nouă) sau pe `/auth/invitatie/[token]` (invitație). Backend-ul apelează `supabase.auth.verifyOtp({ email, token, type: "signup" | "email" | "magiclink" })`. După confirmarea invitației, se scrie `clinic_profiles` și invitația e marcată acceptată; destinația e `/dashboard`. `POST /api/auth/email-otp` primește `{ email, purpose: "register" }` — `purpose: "login"` este respins. Conturile confirmate se autentifică cu `signInWithPassword`, fără un al doilea cod. Fără `RESEND_API_KEY`, în dezvoltare codul e logat și întors ca `devCode`.
