...
- Opțional, pentru **emailuri** (suport + OTP autentificare): `RESEND_API_KEY`. OTP-ul de login/înregistrare clinică pleacă de pe `no-reply@kinetoflow.ro` (`RESEND_AUTH_FROM`) ca **cod de 6 cifre în text**, fără buton de confirmare. Fără cheie, în dezvoltare codul e logat.
...
În dashboard-ul Supabase, **Authentication → Providers → Email** trebuie să fie activ. **Confirm email** poate rămâne pornit: după ce utilizatorul tastează OTP-ul de 6 cifre în `/auth/email-cod`, serverul confirmă adresa cu `verifyOtp` pe același dispozitiv. Nu mai există buton de confirmare în email care să mute sesiunea pe telefonul de pe care s-a deschis inbox-ul. `signUp` rămâne ca rezervă internă; nu trimitem `emailRedirectTo` ca mecanism principal de confirmare. Pentru **Google**, activează providerul Google (Client ID + secret din Google Cloud Console). Redirect-ul din consola Google este `https://<proiect>.supabase.co/auth/v1/callback`; în aplicație, după OAuth, utilizatorul revine pe `/auth/callback`.
...
| `/login` | Interfața clasică: **Google**, apoi email + parolă. După submit, aplicația trimite un **cod de 6 cifre** pe email și deschide `/auth/email-cod`. Tab-uri **Intră în cont** / **Înregistrează clinică nouă**. La signup e obligatoriu consimțământul la Termeni. |
| `/auth/email-cod` | Ecran de confirmare: utilizatorul tastează manual codul de 6 cifre. Validarea e `verifyOtp` pe dispozitivul de pe care a început înregistrarea; linkurile din email nu mai autentifică automat. |
...
OTP de autentificare (admin / terapeut): rulează `sql/026_auth_email_otps.sql` (sau `supabase/migrations/019_auth_email_otps.sql`). La înregistrare, serverul generează OTP-ul prin `auth.admin.generateLink` și îl trimite **doar ca text** (6 cifre vizibile) de pe `KinetoFlow <no-reply@kinetoflow.ro>` (`RESEND_AUTH_FROM`). Nu include un buton/link de autologin. Utilizatorul tastează codul pe `/auth/email-cod`; backend-ul apelează `supabase.auth.verifyOtp({ email, token, type: "signup" | "email" | "magiclink" })`. `POST /api/auth/email-otp` primește `{ email, purpose: "login" | "register" }`. Verificare: `POST /api/auth/email-otp/verify` cu `{ email, code }`. Fără `RESEND_API_KEY`, în dezvoltare codul e logat și întors ca `devCode`. Domain-ul `kinetoflow.ro` trebuie verificat în Resend.
