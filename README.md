...
Tabele: `patients` (token UUID unic pentru `/patient/[token]`; **fără** coloana `clinic_id` — cabinetul e `therapist_id` + `clinic_profiles.clinic_name`), `exercises`, `check_ins`, `exercise_completions` (finalizări zilnice din portalul pacientului — rulează `016_exercise_completions.sql`). Biblioteca din aplicație (`/dashboard/exercises`) rămâne comună; `exercise_library` e catalog, fără date de pacient. Rulează `018_exercise_library_rls.sql`: SELECT pentru toți terapeuții autentificați; INSERT/UPDATE/DELETE doar pentru `kinetic01flow@gmail.com`.
...
- RLS pe `exercise_library`: citire pentru `authenticated`; scriere doar `kinetic01flow@gmail.com` (`018_exercise_library_rls.sql`). Backend-ul verifică același email înainte de INSERT/UPDATE/DELETE.
