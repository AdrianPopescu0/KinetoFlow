...
| Rută | Rol |
| --- | --- |
| `/` | Landing de prezentare, **hero întunecat** (fotografie de recuperare + verde medical). Butoane către `/login` (autentificare) și înregistrare clinică. Secțiuni: **Ce este KinetoFlow?**, cifre cheie pe bandă foto (ședință de kinetoterapie; valorile sunt momentan 0), beneficii, îndemn la acțiune, **Întrebări frecvente** (acordeon) înainte de subsol. Fără cumpărare abonament. |
...
| `/dashboard/setari` | Setări cont (buton **Setări** din bara terapeutului): comutator Dark/Light, mod automat, date personale, parolă și numele clinicii (doar admin) |
...
OTP de confirmare email (**doar la înregistrarea unei clinici noi**, nu la invitația de terapeut): rulează `sql/026_auth_email_otps.sql` (sau `supabase/migrations/019_auth_email_otps.sql`). Fluxul **nu** apelează `signInWithOtp` și **nu** apelează `auth.resend`. Serverul:
...
Utilizatorul tastează codul pe `/auth/email-cod` (**doar clinică nouă**). Invitațiile de terapeut **nu** trimit și **nu** cer OTP. `POST /api/auth/email-otp` primește `{ email, purpose: "register" }` — `purpose: "login"` este respins. Conturile confirmate se autentifică cu `signInWithPassword`, fără un al doilea cod.
