...
- RLS pe `exercise_library`: citire pentru oricine; scriere (INSERT/UPDATE/DELETE) doar `kinetic01flow@gmail.com` și `admin@kinetoflow.ro` (email exact, fără potriviri parțiale). Rulează `021_exercise_library_editors.sql`. Tabela `exercises` (programul pacientului) rămâne editabilă de terapeuții cabinetului.
