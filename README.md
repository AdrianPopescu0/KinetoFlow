...
- Pentru reminder-e și notificări de check-in (**doar Web Push / FCM**): cheile `NEXT_PUBLIC_FIREBASE_*` (inclusiv `NEXT_PUBLIC_FIREBASE_VAPID_KEY`) plus pe server `FIREBASE_SERVICE_ACCOUNT` sau `FIREBASE_PROJECT_ID` + `FIREBASE_CLIENT_EMAIL` + `FIREBASE_PRIVATE_KEY`. Fără token FCM salvat, reminder-ul se sare — nu există fallback SMS sau WhatsApp. Local, fără Firebase Admin, trimiterea push e simulată în loguri.
...
3. Pe Vercel (și în `.env.local`) pune cheile **cu prefix `NEXT_PUBLIC_`**: `API_KEY`, `AUTH_DOMAIN`, `PROJECT_ID`, `STORAGE_BUCKET`, `MESSAGING_SENDER_ID`, `APP_ID`, `VAPID_KEY`. Alias-uri: `NEXT_PUBLIC_FIREBASE_SENDER_ID`, `NEXT_PUBLIC_FIREBASE_MESSAGING_VAPID_KEY`. După ce le adaugi, **redesfășoară** aplicația (cheile publice se citesc la build). Pe server: `FIREBASE_SERVICE_ACCOUNT` (JSON pe un rând) sau `FIREBASE_PROJECT_ID` + `FIREBASE_CLIENT_EMAIL` + `FIREBASE_PRIVATE_KEY`. `FIREBASE_PRIVATE_KEY` e curățată automat: `\n` literale, newline-uri reale, ghilimele, JSON dublu-escapat sau PEM pe un singur rând — aplicația reconstruiește PEM-ul înainte de `cert()`, ca să nu apară `Failed to parse private key` / `ERR_OSSL_UNSUPPORTED`.
